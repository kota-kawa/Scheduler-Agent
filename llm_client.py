...
def call_scheduler_llm(messages: List[Dict[str, str]], context: str) -> Tuple[str, List[Dict[str, Any]]]:
    ...
    base_system_prompt = (
        ...
        "- **期間を跨ぐ予定**: 「〇〇から〇〇まで旅行」「〇〇〜〇〇連続予定」は `create_tasks_in_range` で一括登録してください。`create_custom_task` を日数分繰り返す必要はありません。\n"
        ...
        "- **エラー非開示**: ツール実行エラーや内部エラーメッセージ（`calc_*`, `create_custom_task`, `execution_errors` 等）はユーザーに見せないでください。エラーが出た場合は内部で修正を試み、どうしても解決しない場合のみ「うまく登録できませんでした」と簡潔に伝えてください。\n"
    )
