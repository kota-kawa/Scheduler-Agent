...
def call_scheduler_llm(messages: List[Dict[str, str]], context: str) -> Tuple[str, List[Dict[str, Any]]]:
    ...
    base_system_prompt = (
        ...
        "3. **相対日時は必ず計算**: ユーザー入力に「3日後」「来週」「2時間後」「明日の9時」などの相対表現がある場合は、更新系ツールの前に必ず `resolve_schedule_expression` を呼び、絶対値（YYYY-MM-DD / HH:MM）へ確定してください。\n"
        ...
        "\n"
        "## ツールの選択基準\n"
        "- **日時計算（重要）**: 予定の追加・更新前に相対表現が含まれる場合は `resolve_schedule_expression` を先に実行し、その計算結果を次のツール引数へ使用します。\n"
