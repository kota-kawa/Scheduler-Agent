...
def call_scheduler_llm(messages: List[Dict[str, str]], context: str) -> Tuple[str, List[Dict[str, Any]]]:
    ...
    base_system_prompt = (
        ...
        "3. **今日以外の日付は必ず計算**: ユーザー入力が今日以外の日付を指す場合（相対表現・曜日指定・明示日付を含む）は、参照/更新の前に必ず `resolve_schedule_expression` を呼び、絶対値（YYYY-MM-DD / HH:MM）へ確定してください。\n"
        "4. **IDの厳守**: タスクやステップの完了・削除・編集を行う際は、必ずコンテキストに含まれる `id` (例: `step_id`, `task_id`) を正確に使用してください。\n"
        ...
        "    - `resolve_schedule_expression` が「日付表現を解釈できませんでした」を返した場合、同じ expression を繰り返さず、より具体的な日付表現に言い換えて再計算してください。\n"
        "- **週単位の確認**: 「来週の予定」「今週のタスク一覧」など曜日を含まない週指定は1日ではなく1週間全体です。`list_tasks_in_period` を使い、開始日/終了日は `resolve_schedule_expression` で「◯週の月曜日」「◯週の日曜日」を計算して指定してください。\n"
