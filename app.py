...
def _is_internal_system_error(error_text: str) -> bool:
    # 日本語: 内部制御向けエラーか判定 / English: Detect internal control-flow errors
...
def _looks_mechanical_reply(text: str) -> bool:
    # 日本語: 機械的な生ログっぽい文面の検出 / English: Detect raw/mechanical response patterns
    if not isinstance(text, str):
        return False
...
def _friendly_result_line(result: str) -> List[str]:
    # 日本語: 実行結果1行を親しみやすい表現へ / English: Convert one raw result line into friendly message lines
...
def _build_pop_friendly_reply(
    ...
    # 日本語: ローカルで親しみやすい最終返信を生成 / English: Build friendly fallback reply without extra LLM
    lines: List[str] = []
    lines.append("✨ 実行しました！")
